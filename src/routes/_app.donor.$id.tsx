...
function DonorDetail() {
  ...
  if (isError || !donor) {
    return (
      <div className="card-elevated p-8 text-center">
        תורם לא נמצא.{" "}
        <Link to="/donors" className="text-brand">
          חזרה
        </Link>
      </div>
    );
        ...
        {i.followUpAction && (
          ...
          >
            <span className="flex items-center gap-1">
              ...
              {isOverdue(i.followUpDate) && i.status !== "הושלם" && (
                <strong className="mr-1">· באיחור</strong>
              )}
            </span>
            {i.status !== "הושלם" && (
              <button
  ...
  const interactionFields = [
    ...
    { name: "outcome", label: "תוצאה / החלטות", type: "textarea" as const, colSpan: 2 as const },
    {
      ...
      label: "משימת המשך",
      colSpan: 2 as const,
      helper: "אם תוגדר משימה, האינטראקציה תיפתח כמשימה פעילה ותופיע בהתראות",
    },
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
                <span>{donor.id}</span>·<StatusBadge value={donor.type} />·
                <StatusBadge value={donor.status} />
              </div>
            </div>
          </div>
