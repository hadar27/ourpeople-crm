  ...
  return (
    <div className={`rounded-xl border border-border p-5 shadow-soft ${toneClass}`}>
      <div className="flex items-start justify-between">
        <div>
          <div
            className={`text-xs ${tone === "brand" ? "text-white/80" : "text-muted-foreground"}`}
          >
            {label}
          </div>
          ...
          {delta && (
            <div
              className={`mt-1 text-xs ${tone === "brand" ? "text-white/80" : "text-muted-foreground"}`}
            >
              {delta}
            </div>
          )}
        ...
        {icon && (
          <div
            className={`p-2 rounded-lg ${tone === "brand" ? "bg-white/15" : "bg-secondary text-brand-deep"}`}
          >
            {icon}
          </div>
        )}
...
export function StatusBadge({ value }: { value: string }) {
  const map: Record<string, string> = {
    ...
    "לא הופק": "bg-rose-50 text-rose-700 border-rose-200",
    "לא הופקו": "bg-rose-50 text-rose-700 border-rose-200",
    הופקו: "bg-emerald-50 text-emerald-700 border-emerald-200",
