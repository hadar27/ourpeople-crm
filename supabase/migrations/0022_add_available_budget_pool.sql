...
BEGIN
  ...
  WHILE amount_needed > 0 LOOP
    generated_donation_id := 'DN-POOL-' || LPAD(donation_index::text, 4, '0');
    WHILE EXISTS (
      SELECT 1 FROM public.donations d WHERE d.id = generated_donation_id
    ) LOOP
